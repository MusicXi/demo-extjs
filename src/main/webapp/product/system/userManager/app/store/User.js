Ext.define('UserManager.store.User', {
	extend: 'Ext.data.Store',
	model:'UserManager.model.User',//store的配置model必须要是类完整的名称,否则store对象无法引用到
	autoLoad:true,//自动载入
	//remoteFilter:true, 
	remoteSort:true,
	pageSize:15,	//设置分页大小
	proxy: {
		...
		api:{
			read:webRoot+'/product/system/userManager/read.do',
			create:webRoot+'/product/system/userManager/create.do',
			update:webRoot+'/product/system/userManager/update.do',
			destroy:webRoot+'/product/system/userManager/destroy.do'
		},
